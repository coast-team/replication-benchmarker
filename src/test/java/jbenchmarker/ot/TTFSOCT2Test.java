...
public class TTFSOCT2Test {
    ...
    public void testGenerateLocalInsertCharByChar() throws IncorrectTraceException {
        ...
        List<SequenceMessage> ops = merger.generateLocal(insert( 0, "b"));
        ...
        ops = merger.generateLocal(insert( 0, "a"));
        assertEquals(1, ops.size());
        ...
        ops = merger.generateLocal(insert( 2, "c"));
        assertEquals(1, ops.size());
        opg = ((TTFSequenceMessage) ops.get(0)).getSoct2Message();
    ...
    public void testGenerateLocalInsertString() throws IncorrectTraceException {
        ...
        List<SequenceMessage> ops = merger.generateLocal(insert( 0, "abc"));
    ...
    public void testGenerateLocalDeleteCharByChar() throws IncorrectTraceException {
        int siteId = 0;
        TTFMergeAlgorithm merger = new TTFMergeAlgorithm(new TTFDocument(), siteId);
        merger.generateLocal(insert( 0, "abcd"));

        // remove 'a'
        List<SequenceMessage> ops = merger.generateLocal(delete( 0, 1));
        assertEquals(1, ops.size());
        OTMessage<TTFOperation> opg = ((TTFSequenceMessage) ops.get(0)).getSoct2Message();
        ...
        // remove 'd'
        ops = merger.generateLocal(delete( 2, 1));
        assertEquals(1, ops.size());
        opg = ((TTFSequenceMessage) ops.get(0)).getSoct2Message();
        assertEquals(OpType.del, opg.getOperation().getType());
        ...
        // remove 'c'
        ops = merger.generateLocal(delete( 1, 1));
        assertEquals(1, ops.size());
        opg = ((TTFSequenceMessage) ops.get(0)).getSoct2Message();
        assertEquals(OpType.del, opg.getOperation().getType());
        assertEquals(2, opg.getOperation().getPosition());
        ...
        // remove 'b'
        ops = merger.generateLocal(delete( 0, 1));
        assertEquals(1, ops.size());
        opg = ((TTFSequenceMessage) ops.get(0)).getSoct2Message();
        assertEquals(OpType.del, opg.getOperation().getType());
        assertEquals(1, opg.getOperation().getPosition());
    ...
    public void testGenerateLocalDeleteString() throws IncorrectTraceException {
        ...
        // remove "abcd"
        List<SequenceMessage> ops = merger.generateLocal(delete( 0, 4));
    ...
    public void testGenerateLocalDeleteStringWhichContainsDeletedChars() throws IncorrectTraceException {
        int siteId = 0;
        TTFMergeAlgorithm merger = new TTFMergeAlgorithm(new TTFDocument(), siteId);
        merger.generateLocal(insert( 0, "abcdefg"));

        merger.generateLocal(delete( 2, 2));
        assertEquals("abefg", merger.lookup());

        // remove "bef"
        List<SequenceMessage> ops = merger.generateLocal(delete( 1, 3));
    ...
    public void testVectorClockEvolution() throws IncorrectTraceException {
        ...
        OTMessage op1 = ((TTFSequenceMessage) merger.generateLocal(insert( 0, "a")).get(0)).getSoct2Message();
        assertEquals(vc(1), merger.getClock());

        OTMessage op2 = ((TTFSequenceMessage) merger.generateLocal(insert( 1, "b")).get(0)).getSoct2Message();
    ...
    public void testTP2PuzzleAtSite0() throws IncorrectTraceException {
        ...
        site0.generateLocal(insert( 0, "abc"));
    ...
    public void testTP2PuzzleAtSite1() throws IncorrectTraceException {
        TTFMergeAlgorithm site0 = new TTFMergeAlgorithm(new TTFDocument(), 0);
        List<SequenceMessage> ops = site0.generateLocal(insert( 0, "abc"));
    ...
    public void testTP2Puzzle() throws IncorrectTraceException {
        ...
        List<SequenceMessage> ops0 = duplicate(site1.generateLocal(insert( 0, "abc")));
        ...
        List<SequenceMessage> ops1 = duplicate(site1.generateLocal(insert( 1, "x")));
        assertEquals("axbc", site1.lookup());
        List<SequenceMessage> ops2 = duplicate(site2.generateLocal(insert( 2, "y")));
        assertEquals("abyc", site2.lookup());
        List<SequenceMessage> ops3 = duplicate(site3.generateLocal(delete( 1, 1)));
    ...
    public void testBasicScenario() throws IncorrectTraceException {
        ...
        List<SequenceMessage> ops0 = duplicate(site0.generateLocal(insert( 0, "ABC")));
        assertEquals("ABC", site0.lookup());

        integrateSeqAtSite(ops0, site2);
        assertEquals("ABC", site2.lookup());

        List<SequenceMessage> ops2 = duplicate(site2.generateLocal(insert( 2, "vw")));
        assertEquals("ABvwC", site2.lookup());
        List<SequenceMessage> ops2b = duplicate(site2.generateLocal(insert( 4, "xyz")));
        assertEquals("ABvwxyzC", site2.lookup());

        integrateSeqAtSite(ops0, site4);
        assertEquals("ABC", site4.lookup());

        List<SequenceMessage> ops4 = duplicate(site4.generateLocal(delete( 1, 2)));
    ...
    public void testBasicScenario2() throws IncorrectTraceException {
         ...
         * 4, [<0,1><4,1>])
         */
        List<SequenceMessage> ops0 = duplicate(site0.generateLocal(insert(0, "Salut Monsieur  \nFin"))); // [<0,1>]        

        integrateSeqAtSite(ops0, site2);
        List<SequenceMessage> ops2 = site2.generateLocal(insert( 14, "Bonjour")); // [<2,1><0,1>]        
        List<SequenceMessage> ops2b = site2.generateLocal(insert( 21, " Mehdi")); // [<2,2><0,1>]
        assertEquals("Salut MonsieurBonjour Mehdi  \nFin", site2.lookup());

        integrateSeqAtSite(ops0, site4);
        List<SequenceMessage> ops4 = duplicate(site4.generateLocal(delete( 14, 3))); // [<0,1><4,1>]
...
//        List<SequenceMessage> ops2 = duplicate(site2.generateLocal(insert(2, 0, "ed", vc(0,0,1,0))));
        ...
        vc2.inc(2);
        List<SequenceMessage> ops2 = duplicate(site2.generateLocal(insert( 0, "ed")));
...
//        List<SequenceMessage> ops3 = duplicate(site3.generateLocal(insert(3, 0, "h", vc(0,0,0,1))));
        ...
        vc3.inc(3);
        List<SequenceMessage> ops3 = duplicate(site3.generateLocal(insert( 0, "h")));
...
//        List<SequenceMessage> ops1 = duplicate(site1.generateLocal(insert(1, 0, "q", vc(0,1,0,0))));
        ...
        vc1.inc(1);
        List<SequenceMessage> ops1 = duplicate(site1.generateLocal(insert( 0, "q")));
    ...
    public void testPartialConcurrencyScenarioWith3Insert() throws IncorrectTraceException {
        ...
        List<SequenceMessage> ops1 = duplicate(site1.generateLocal(insert( 2, "X")));
        assertEquals("ABXC", site1.lookup());

        List<SequenceMessage> ops2 = duplicate(site2.generateLocal(insert( 1, "12")));
        List<SequenceMessage> ops2b = duplicate(site2.generateLocal(insert( 3, "34")));
