...
import jbenchmarker.RGALogootSplitTree.RGALogootSplitTreeFactory;
...
public class RgaSDocumentTest {
	...
	public void setUp() throws Exception {
		replica = (RgaSMerge) new RGALogootSplitTreeFactory().create(REPLICA_ID);
	}
	...
	public void testGetPosition() {
		...
		RgaSS3Vector s3v1 = new RgaSS3Vector(0,0,1,0);
		RgaSS3Vector s3v2 = new RgaSS3Vector(0,0,2,0);
		RgaSS3Vector s3v3 = new RgaSS3Vector(0,0,3,0);
...
/*
	...
	public void testFindGoodNode() {
		...
		RgaSNode test;
		RgaSS3Vector s3v1 = new RgaSS3Vector(0,0,1,0);
		RgaSS3Vector s3v2 = new RgaSS3Vector(0,0,2,2);
		RgaSS3Vector s3v3 = new RgaSS3Vector(0,0,3,5);
		RgaSS3Vector s3v4 = new RgaSS3Vector(0,0,4,7);
		RgaSS3Vector s3v5 = new RgaSS3Vector(0,0,5,14);
...
*/
	...
	public void testRemoteSplit() throws PreconditionException {
		...
		rgadoc.getRga().remoteSplit(rgadoc.getHead().getNext(), 0);
		assertEquals("Split au début","->|abcdefghijklmnopq|", rgadoc.viewChainWithSeparator());
		assertEquals("Split au début",0, rgadoc.getHead().getNext().getNodeID().getOffset());

		rgadoc.getRga().remoteSplit(rgadoc.getHead().getNext(), 17);
		assertEquals("Split à la fin","->|abcdefghijklmnopq|", rgadoc.viewChainWithSeparator());
		assertEquals("Split au début",0, rgadoc.getHead().getNext().getNodeID().getOffset());

		rgadoc.getRga().remoteSplit(rgadoc.getHead().getNext(), 9);
		assertEquals("Split au milieu","->|abcdefghi|->|jklmnopq|", rgadoc.viewChainWithSeparator());
		assertEquals("Verif offset",9, rgadoc.getHead().getNext().getNext().getNodeID().getOffset());

		rgadoc.getRga().remoteSplit(rgadoc.getHead().getNext().getNext(), 14);
		assertEquals("Split au milieu","->|abcdefghi|->|jklmn|->|opq|", rgadoc.viewChainWithSeparator());
		assertEquals("Verif offset",9, rgadoc.getHead().getNext().getNext().getNodeID().getOffset());
		assertEquals("Verif offset",14, rgadoc.getHead().getNext().getNext().getNext().getNodeID().getOffset());
	...
	public void testRemoteInsert() throws PreconditionException {    
		...
		replica.applyLocal(SequenceOperation.insert(5, "fg"));
		assertEquals("Insertion au milieu","abcdefgjk", replica.lookup()); 

		replica.applyLocal(SequenceOperation.insert(7, "hi"));
		assertEquals("Insertion au milieu","abcdefghijk", replica.lookup());
		
		replica.applyLocal(SequenceOperation.insert(11, "lmnop"));	
		assertEquals("Insertion à la fin","abcdefghijklmnop", replica.lookup()); 
