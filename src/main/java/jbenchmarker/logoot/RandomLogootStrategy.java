...
/**
 * Logoot strategy that uses integer range position and random to create new identifier.
 * @author urso
 */
public abstract class RandomLogootStrategy implements Serializable, LogootStrategy {
    ...
    static LogootIdentifier constructIdentifier(List<Long> digits, LogootIdentifier P, LogootIdentifier Q, int peer, int clock) {
        ...
        while (i <= index) {
            R.addComponent(new LogootComponent(digits.get(i), peer, clock));
            i++;
        }
        return R;
    }
