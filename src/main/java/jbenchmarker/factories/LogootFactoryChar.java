...
import jbenchmarker.logoot.LogootDocumentChar;
 ...
 * @author urso
 */
public class LogootFactoryChar<T> extends ReplicaFactory {
    @Override
    public LogootMerge<T> create(int r) {         
        return new LogootMerge<T>(createDoc(r, 64, 1000000000), r);
    }
    
    static public LogootDocumentChar createDoc(int r, int base, int bound) {
        LogootStrategy s = new BoundaryStrategy(base, bound);
        return new LogootDocumentChar(r, s); 
    }
}
